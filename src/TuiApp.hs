...
drawMailBox ts = viewport ResourceName Vertical $ borderWithLabel (makeVisib $ str $ mailBoxLabel (inMailbox ts)) $ vBox $ map (drawEntry (showDesc ts) (selectedItem ts)) (zip (entries ts) [0,1..])
  where
